...
export default async function handler(req, res) {
    ...
    const body = req.body;
    const isSignatureValid = validateSignature(body.address, 'save-project', body.signature, req, res);
    ...
    if (isSignatureValid) {
        ...
        if (mode === 'create') {
            ...
            const payload = { $push: { projects: project } };
            ...
            // delete project
            const projectId = new ObjectId(project.id);
            const payload = { $pull: { projects: { id: projectId } } };
            await accounts.updateOne({ address: body.address }, payload);
        }
